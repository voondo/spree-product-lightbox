module ProductImageHelper
  def link_to_main_product_image(product)
    ...
    link_to product_image(product), large_image_url, :title => product.name#, :itemprop => "image"
  end
  
  def link_to_image(image, title='')
    link_to image_tag(image.url(:mini)), image.url(:large), :title => title, :class => 'full_img'#, :itemprop => "image"
  end
end
